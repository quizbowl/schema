...
The top level of the JSON file **must** be an object with keys for `version` (the current version is `1.4`), and `objects`, an array of objects; among these objects, there should be exactly one object of type `Tournament` and any number of other objects.
